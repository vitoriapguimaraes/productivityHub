...
if uploaded_files:
    ...
    file_list = [{"Arquivo": f.name, "Ordem": i+1} for i, f in enumerate(uploaded_files)]
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "Ordem": st.column_config.NumberColumn("Ordem (1=Primeiro)", min_value=1, max_value=len(uploaded_files), step=1)
        },
        hide_index=True,
    ...
    if st.button("Unificar PDFs nesta Ordem 🚀", type="primary"):
        ...
        if edited_df["Ordem"].duplicated().any():
            st.error("❌ Erro: Existem números de ordem repetidos. Por favor, use uma sequência única (ex: 1, 2, 3...).")
        else:
